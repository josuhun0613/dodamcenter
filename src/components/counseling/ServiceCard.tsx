...
export default function ServiceCard({ title, description, price, duration, href, icon }: ServiceCardProps) {
  return (
    <Link href={href} className="group block">
      <div className="bg-white rounded-2xl border border-beige-200 p-8 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-accent/30">
        <div className="w-14 h-14 rounded-xl bg-beige-100 text-accent flex items-center justify-center mb-6 group-hover:bg-accent group-hover:text-white transition-all duration-300">
          {icon}
        </div>
