    ...
    <div>
      ...
      {inquiries.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {inquiries.map((inquiry) => (
              <button
                ...
                className={`w-full text-left bg-white rounded-xl border p-4 transition-all hover:shadow-md cursor-pointer ${
                  selectedInquiry?.id === inquiry.id ? 'border-accent ring-1 ring-accent' : 'border-sage-200'
                } ${!inquiry.isRead ? 'border-l-4 border-l-accent' : ''}`}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-black">{inquiry.name}</span>
                  <span className="text-xs text-sage-300">{new Date(inquiry.createdAt).toLocaleDateString('ko-KR')}</span>
                </div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs px-2 py-0.5 rounded-full bg-sage-50 text-accent">{inquiry.counselingType}</span>
          ...
          {/* Detail */}
          {selectedInquiry ? (
            <div className="bg-white rounded-xl border border-sage-200 p-6 sticky top-24">
              <h2 className="text-lg font-semibold text-black mb-4">문의 상세</h2>
              <div className="space-y-3">
                <div>
                  <span className="text-xs text-sage-300 uppercase">이름</span>
                  <p className="text-black font-medium">{selectedInquiry.name}</p>
                </div>
                <div>
                  <span className="text-xs text-sage-300 uppercase">연락처</span>
                  <p className="text-black">{selectedInquiry.phone}</p>
                </div>
                {selectedInquiry.email && (
                  <div>
                    <span className="text-xs text-sage-300 uppercase">이메일</span>
                    <p className="text-black">{selectedInquiry.email}</p>
                  </div>
                )}
                <div>
                  <span className="text-xs text-sage-300 uppercase">상담 유형</span>
                  <p className="text-accent font-medium">{selectedInquiry.counselingType}</p>
                </div>
                <div>
                  <span className="text-xs text-sage-300 uppercase">내용</span>
                  <p className="text-black-light whitespace-pre-wrap leading-relaxed">{selectedInquiry.message}</p>
                </div>
                <div>
                  <span className="text-xs text-sage-300 uppercase">접수일</span>
                  <p className="text-black-light">{new Date(selectedInquiry.createdAt).toLocaleString('ko-KR')}</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-xl border border-sage-200 p-12 text-center flex items-center justify-center">
